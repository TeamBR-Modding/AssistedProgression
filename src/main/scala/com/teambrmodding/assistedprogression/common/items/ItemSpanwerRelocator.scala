...
/**
  ...
  * @since 2/2/2016
  */
class ItemSpanwerRelocator extends BaseItem("itemSpawnerRelocator", 1) {
    ...
    override def addInformation(stack: ItemStack, player: EntityPlayer, list: java.util.List[String], boolean: Boolean): Unit = {
        if (stack.hasTagCompound) {
            list.add("" + GuiColor.ORANGE + GuiTextFormat.ITALICS + "Type: " + stack.getTagCompound.getTagList("SpawnPotentials", 10).getStringTagAt(0).split("\"")(1))
        }
    }
}
