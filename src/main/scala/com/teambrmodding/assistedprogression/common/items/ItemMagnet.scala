...
/**
  ...
  * @since 2/19/2016
  */
class ItemMagnet(var evil : Boolean, var name : String) extends ItemBattery {
    lazy val RANGE = 10D
    ...
    override def setDefaultTags(stack: ItemStack): Unit = {
        if (!stack.hasTagCompound && !evil) {
            val tag = new NBTTagCompound
            tag.setInteger("Energy", 0)
            tag.setInteger("EnergyCapacity", ConfigManager.totalRFMagnet)
    ...
    override def onItemRightClick(stack: ItemStack, world: World, player: EntityPlayer, hand: EnumHand) : ActionResult[ItemStack] = {
        if(player.isSneaking) {
            if(!stack.hasTagCompound) {
                ...
                if(!world.isRemote)
                    player.addChatComponentMessage(new TextComponentString(I18n.translateToLocal("assistedprogression.text.magnetism") + ": " +
                            (if(!active) GuiColor.GREEN + I18n.translateToLocal("assistedprogression.text.active")
                            else GuiColor.RED + I18n.translateToLocal("assistedprogression.text.disabled"))))
            }
        }
                ...
                if(stack.hasTagCompound) {
                    ...
                    if(stack.getTagCompound.getBoolean("Active") && (energy > 0 || evil)) {
                        ...
                        if(!entities.isEmpty) {
                            ...
                            //Moved stuff, drain
                            if(!evil)
                                stack.getItem.asInstanceOf[ItemMagnet].extractEnergy(stack, 1, simulate = false)
                        }
                    }
                }
            case _ =>
    ...
    override def addInformation(stack: ItemStack, player: EntityPlayer, list: java.util.List[String], boolean: Boolean): Unit = {
        if(stack.hasTagCompound) {
            val active = stack.getTagCompound.getBoolean("Active")
            list.add(if (active) GuiColor.GREEN + I18n.translateToLocal("assistedprogression.text.active")
            else GuiColor.RED + I18n.translateToLocal("assistedprogression.text.disabled"))

            if(!evil) {
                list.add(GuiColor.ORANGE + I18n.translateToLocal("assistedprogression.text.redstoneFlux"))
                ...
                list.add(GuiColor.ORANGE + I18n.translateToLocal("assistedprogression.text.magnetEvil"))
            }
        }
    }
}
