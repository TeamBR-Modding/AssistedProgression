package com.teambrmodding.assistedprogression.common.container.slot
...
/**
  ...
  * @since 1/26/2016
  */
class SlotNull(var size: Int, slotSize : SLOT_SIZE.Value, inventory : IInventory, id : Int, x : Int, y : Int)
    ...
    override def getSlotTexture : String = {
        if(size <= 1)
            "assistedprogression:items/trashBag"
        else
            "assistedprogression:items/heftyBag"
    }
}
